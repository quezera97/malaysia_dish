import '../../popular_sweets_enum.dart';

Map<String, String> kelantanSweetsUrl = {
  PopularSweetsEnum.akok:
      'https://youtu.be/gi3mTsFKoQ4?si=QTiEuzYY6hls95Aj',
  PopularSweetsEnum.butirNangka: 'https://youtu.be/KGWhgxs7mBM?si=Xplphbz-UJ-K1Iz5',
  PopularSweetsEnum.cekMekMolek:
      'https://youtu.be/K9i3TVKg4oQ?si=Zh171I8cj2QgR2-_',
  PopularSweetsEnum.lompatTikam:
      'https://youtu.be/bTa8GRhkC60?si=39q7pSc6yREM1-IP',
  PopularSweetsEnum.kuihKato: 'https://youtu.be/Fb2nJJtPNIU?si=2JbWF09O2BqV7_oS',
};
